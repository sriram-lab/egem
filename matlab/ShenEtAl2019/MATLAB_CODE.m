...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       %% impact of basal metabolic state of CCLE cell lines on sensitivity to deacetylase inhibitors
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% intialize params for constrain_flux_regulation
MODE = 1;  % reaction (1) or gene list (0)
epsilon = 1E-2; 
rho = 1;
...
hdactransint = 1;
basalflag = 1;
epsilon_methylation = 1E-2;
rxnpos  = find(ismember(metabolicmodel.rxns,'LYSMTF1n')); % rxnpos = 2451

% create empty matrices. will be populated later.
...
% match cell line data in CCLE with CTD2 to get gene expression data
for i = 1:length(exptidcelllinemediamatch)
    ...
    if ~isempty(iii)
        ...
        % Create 6 medium conditions to test by changing the substrate
        % uptake rate of glucose using the amount of glucose available. 
        % default glucose is -5 for rpmi
        if r1(i)
            model2.lb(find(ismember(model2.rxns, {'EX_glc(e)'})))  = -5;% no change rpmi
        elseif r2(i)
        ...
        if basalflag
            % impact on growth
            [fluxstate_gurobi, grate_ccle_exp_soft(i,1), solverobj_ccle(i,1)] =  constrain_flux_regulation(model2,onreactions,offreactions,kappa,rho,epsilon,MODE,[], minfluxflag);
            % Impact on methylation
            model2.c(rxnpos) = epsilon_methylation;
            [fluxstate_gurobi] =  constrain_flux_regulation(model2,onreactions,offreactions,kappa,rho,epsilon,MODE,[],minfluxflag);
            grate_ccle_exp_soft(i,2) = fluxstate_gurobi(rxnpos); 
        end
        
        % hdac inhibitor impact on transcriptome
        if hdactransint
            for kk = 4:-1:1   % match drug order. This also does it backwards.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%predicting variation in sensitivity between hdac inhibitors based on basal metabolic state and drug impact - comparison with drug sensitivity data from seashore-ludlow study
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
