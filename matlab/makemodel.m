...
model = addReaction(model, 'ACtn',...
    'reactionFormula', 'ac[c] <=> ac[n]');
model = addReaction(model, 'PYRtn',...
...
model = addReaction(model, 'MAT2n',...
    'reactionFormula', 'atp[n] + h2o[n] + met__L[n] -> amet[n] + pi[n] + ppi[n]',...
    'geneRule', '(MAT2A)');
% HCy Production reaction
model = addReaction(model, 'AHCn',...
    'reactionFormula', 'ahcys[n] + h2o[n] -> adn[n] + hcys__L[n]',...
    'geneRule', '(AHCY) or (AHCYL1) or (AHCYL1)');
% CBS
model = addReaction(model, 'CYSTSn',...
    'reactionFormula', 'hcys[n] + ser__L[n] -> cyst__L[n] + h2o[n]',...
    'geneRule', '(CBS)');
% BHMT (somewhat - not dominant nuclear mechanism)
model = addReaction(model, 'BHMTn',...
    'reactionFormula', 'glyb[n] + hcys__L[n] -> met__L[n] + dmgly[n]',...
