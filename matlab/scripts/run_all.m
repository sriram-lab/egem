...
% Optimization 2: Run Flux balance analysis (FBA) w/ and w/o competition
medium_of_interest = {'RPMI', 'DMEM', 'L15'};
for med = 1:length(medium_of_interest)
    disp(medium_of_interest(med))
    
    % Run all w/o competition
    str =  strcat("[~, fba_", lower(medium_of_interest(med)),"_noComp, ~, ~, ~] =metabolic_sensitivity(model, 'n', epsilon2_", lower(medium_of_interest(med)), ", 'zscore', 'fba', medium_of_interest(med), []);");
    eval(str)
    str = strcat("plot_heatmap(fba_", lower(medium_of_interest(med)), "_noComp, 'fba', epsilon2, medium_of_interest(med))");
    eval(str)
    
    % Run all w/ competition for all reactions
    str =  strcat("[~, fba_", lower(medium_of_interest(med)),"_compAll, ~, ~, ~] =metabolic_sensitivity(model, 'n', epsilon2_", lower(medium_of_interest(med)), ", 'zscore', 'fba', medium_of_interest(med), []);");
    eval(str)
    str = strcat("plot_heatmap(fba_", lower(medium_of_interest(med)), "_compAll, 'fba', epsilon2, medium_of_interest(med))");
    eval(str)
    
    % Run all w/ competition for histone reactions only
    str =  strcat("[~, fba_", lower(medium_of_interest(med)),"_compHist, ~, ~, ~] =metabolic_sensitivity(model, 'n', epsilon2_", lower(medium_of_interest(med)), "(2:5,:), 'zscore', 'fba', medium_of_interest(med), []);");
    eval(str)
    str = strcat("plot_heatmap(fba_", lower(medium_of_interest(med)), "_compHist, 'fba', epsilon2, medium_of_interest(med))");
    eval(str)
end
