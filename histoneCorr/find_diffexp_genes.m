function [diffExp_genes] = find_diffexp_genes(model, matchedCells)
    
    load ccle_geneExpression_vars;
    diffExp_genes.name = [];
    
    for cell = 1:length(matchedCells)
        cellLine = matchedCells(cell);
        cell_line_positions = find(contains(string(celllinenames_ccle1), ...
            string(cellLine)));
