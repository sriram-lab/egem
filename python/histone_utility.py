...
def column_remove(df, list_columns):    
    """Remove any unneeded columns of dataframes being created by the 
    read_csv_df function
    
    INPUT
    
        df: dataframe which you would like to remove column from
        
        list_columns: list of columns which you would like to remove
                      from dataframe
    
    OUTPUT
    
        df: dataframe with out the columns you wished to remove
    """
...
def gene_converstion(gene_list, scopes, fields, species):
    ...
    The gene_list may require some editting.
    
    The output is a list of dictionary which contain ids and gene symbols. 
    
    Further manipulation is required regarding removing any duplpicate ids.
