...
First let's compute PCs and visualize the data with classical PCA. 
```{r}
pc = PCAtools::pca(data.matrix(ccle[, 2:ncol(ccle)]))
PCAtools::screeplot(pc)
PCAtools::biplot(pc)
PCAtools::pairsplot(pc)
```

Then let's get the data we want with robust PCA
```{r}
# Generate a Robust Huber object
pc = PcaHubert(data.matrix(ccle[, 2:ncol(ccle)]))

# Get the outliers
filtered_ccle = ccle
filtered_ccle = filtered_ccle[pc@flag, ]
```

### Data Normalization 
Now let's perform upper quartile normalization and log transformation on the counts.
```{r}
gene_symbols = filtered_ccle[, 1]
tmp = data.matrix(filtered_ccle[, 2:ncol(filtered_ccle)])
print(ncol(tmp))
print(nrow(tmp))

# Construct an object for limma-voom
X = DGEList(counts=tmp, genes=gene_symbols, group=as.factor(tissue[2:length(tissue)]))
...
```{r}
# DELL
savepath = "D:\\Data\\RNASeq\\CCLE\\CCLE_CPM.rds"
